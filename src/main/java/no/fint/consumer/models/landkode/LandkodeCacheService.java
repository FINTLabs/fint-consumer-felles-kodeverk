...
@Slf4j
@Service
public class LandkodeCacheService extends CacheService<FintResource<Landkode>> {
    ...
    public LandkodeCacheService() {
        super(MODEL, IsoActions.GET_ALL_LANDKODE);
    }
    ...
    public Optional<FintResource<Landkode>> getLandkode(String orgId, String SystemId) {
        return getOne(orgId, (fintResource) -> fintResource.getResource().getSystemId().getIdentifikatorverdi().equals(SystemId));
    }

	@Override
    public void onAction(Event event) {
        update(event, new TypeReference<List<FintResource<Landkode>>>() {
        });
    }
}
