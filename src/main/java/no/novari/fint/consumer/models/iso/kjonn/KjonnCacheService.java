...
import no.novari.fint.model.felles.kodeverk.iso.Kjonn;
...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.kjonn", havingValue = "false", matchIfMissing = true)
    ...
    public void onAction(Event event) {
        ...
        if (IsoActions.valueOf(event.getAction()) == IsoActions.UPDATE_KJONN) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<KjonnResource>> cacheObjects = data
                    .stream()
