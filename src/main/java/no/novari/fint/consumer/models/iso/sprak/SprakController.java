package no.novari.fint.consumer.models.iso.sprak;
...
public class SprakController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Sprak cache is disabled.");
        }
    ...
    public SprakResources getSprak(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSprakByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Sprak cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<SprakResource> resources;
    ...
    @PostMapping("/$query")
    public SprakResources getSprakByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSprak(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SprakResources getSprakByOdataFilter(
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SprakResources();
    
        ArrayList<SprakResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SprakResource>>() {});
    ...
    public SprakResource getSprakBySystemId(
        ...
        if (cacheService != null) {
            ...
            SprakResource sprak = objectMapper.convertValue(response.getData().get(0), SprakResource.class);
