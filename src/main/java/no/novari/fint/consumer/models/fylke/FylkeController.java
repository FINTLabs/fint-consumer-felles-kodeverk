package no.novari.fint.consumer.models.fylke;
...
public class FylkeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fylke cache is disabled.");
        }
    ...
    public FylkeResources getFylke(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFylkeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fylke cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<FylkeResource> resources;
    ...
    @PostMapping("/$query")
    public FylkeResources getFylkeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFylke(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FylkeResources getFylkeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_FYLKE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FylkeResources();
    
        ArrayList<FylkeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FylkeResource>>() {});
    ...
    public FylkeResource getFylkeBySystemId(
        ...
        if (cacheService != null) {
            ...
            FylkeResource fylke = objectMapper.convertValue(response.getData().get(0), FylkeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(fylke);
        }    
    }
