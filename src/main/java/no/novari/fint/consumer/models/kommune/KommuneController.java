package no.novari.fint.consumer.models.kommune;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Kommune", value = RestEndpoints.KOMMUNE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class KommuneController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Kommune cache is disabled.");
        }
    ...
    @GetMapping
    public KommuneResources getKommune(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKommuneByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Kommune cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<KommuneResource> resources;
    ...
    @PostMapping("/$query")
    public KommuneResources getKommuneByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKommune(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KommuneResources getKommuneByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_KOMMUNE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KommuneResources();
    
        ArrayList<KommuneResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KommuneResource>>() {});
    ...
    public KommuneResource getKommuneBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<KommuneResource> kommune = cacheService.getKommuneBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return kommune.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            KommuneResource kommune = objectMapper.convertValue(response.getData().get(0), KommuneResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(kommune);
        }    
    }
