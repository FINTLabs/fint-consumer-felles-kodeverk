package no.novari.fint.consumer.models.iso.landkode;
...
import no.novari.fint.model.resource.felles.kodeverk.iso.LandkodeResources;
...
public class LandkodeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Landkode cache is disabled.");
        }
    ...
    public LandkodeResources getLandkode(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getLandkodeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Landkode cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<LandkodeResource> resources;
    ...
    @PostMapping("/$query")
    public LandkodeResources getLandkodeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getLandkode(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private LandkodeResources getLandkodeByOdataFilter(
        ...
        Event event = new Event(
                ...
                IsoActions.GET_LANDKODE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new LandkodeResources();
    
        ArrayList<LandkodeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<LandkodeResource>>() {});
    ...
    public LandkodeResource getLandkodeBySystemId(
        ...
        if (cacheService != null) {
            ...
            LandkodeResource landkode = objectMapper.convertValue(response.getData().get(0), LandkodeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(landkode);
        }    
    }
