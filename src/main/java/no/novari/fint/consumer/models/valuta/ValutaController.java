package no.novari.fint.consumer.models.valuta;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Valuta", value = RestEndpoints.VALUTA, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ValutaController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private ValutaCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Valuta cache is disabled.");
        }
    ...
    @GetMapping
    public ValutaResources getValuta(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getValutaByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Valuta cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_VALUTA, client);
        ...
        Stream<ValutaResource> resources;
    ...
    @PostMapping("/$query")
    public ValutaResources getValutaByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getValuta(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ValutaResources getValutaByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_VALUTA, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ValutaResources();
    
        ArrayList<ValutaResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ValutaResource>>() {});
    ...
    @GetMapping("/bokstavkode/{id:.+}")
    public ValutaResource getValutaByBokstavkode(
        ...
        log.debug("bokstavkode: {}, OrgId: {}, Client: {}", id, orgId, client);

        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_VALUTA, client);
        event.setOperation(Operation.READ);
        event.setQuery("bokstavkode/" + id);

        if (cacheService != null) {
            fintAuditService.audit(event);
            fintAuditService.audit(event, Status.CACHE);

            Optional<ValutaResource> valuta = cacheService.getValutaByBokstavkode(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return valuta.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ValutaResource valuta = objectMapper.convertValue(response.getData().get(0), ValutaResource.class);
    ...
    @GetMapping("/nummerkode/{id:.+}")
    public ValutaResource getValutaByNummerkode(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client) throws InterruptedException {
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        log.debug("nummerkode: {}, OrgId: {}, Client: {}", id, orgId, client);

        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_VALUTA, client);
        event.setOperation(Operation.READ);
        event.setQuery("nummerkode/" + id);

        if (cacheService != null) {
            fintAuditService.audit(event);
            fintAuditService.audit(event, Status.CACHE);

            Optional<ValutaResource> valuta = cacheService.getValutaByNummerkode(orgId, id);
