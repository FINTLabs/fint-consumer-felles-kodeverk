package no.novari.fint.consumer.models.iso.kjonn;
...
public class KjonnController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Kjonn cache is disabled.");
        }
    ...
    public KjonnResources getKjonn(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKjonnByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Kjonn cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<KjonnResource> resources;
    ...
    @PostMapping("/$query")
    public KjonnResources getKjonnByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKjonn(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KjonnResources getKjonnByOdataFilter(
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KjonnResources();
    
        ArrayList<KjonnResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KjonnResource>>() {});
    ...
    public KjonnResource getKjonnBySystemId(
        ...
        if (cacheService != null) {
            ...
            KjonnResource kjonn = objectMapper.convertValue(response.getData().get(0), KjonnResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(kjonn);
        }    
    }
